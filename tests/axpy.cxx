...
#include <scalapackpp/pblas/axpy.hpp>

SCALAPACKPP_TEST_CASE( "Axpy", "[axpy]" ) {
  ...
  for(int j = 0; j < N; ++j) {
    ...
    paxpy(M, 2.0, C_sca.data(), 1, 1, C_sca.desc(), 1, A_sca.data(), 1, j+1, A_sca.desc(), 1);
    ...
    if( grid.ipr() == 0 and grid.ipc() == 0 ) {
      ...
      for(int k = 0; k < N; ++k)
      for(int i = 0; i < M; ++i) {
        if(k == j) CHECK( std::abs(B[i + k*M] - TestType(2.) * A[i] - A[i + k*M]) < tol );
