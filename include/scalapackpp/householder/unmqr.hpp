...
detail::enable_if_scalapack_complex_supported_t<T, int64_t>
  punmqr( SideFlag side, TransposeFlag trans, int64_t M, int64_t N, 
  ...
  wrappers::punmqr( SIDE.c_str(), TRANS.c_str(), M, N, A, IA, JA, DESCA, TAU,
                    C, IC, JC, DESCC, WORK.data(), LWORK );

  LWORK = int64_t( std::real(WORK[0]) );
  WORK.resize( LWORK );

  return wrappers::punmqr( SIDE.c_str(), TRANS.c_str(), M, N, A, IA, JA, DESCA, TAU,
                           C, IC, JC, DESCC, WORK.data(), LWORK );
